...
namespace MariaCpp {
    ...
    void* Bind::Buffer::data(const bool& heap) const {
        return heap ? _data : (void*) this;
    }

    size_t Bind::Buffer::size(const bool& heap) const {
        return heap ? _size : sizeof(Buffer);
    }

    void* Bind::Buffer::alloc(bool& heap, size_t len) {
        if (!heap) {
        ...
        if (!_data)
            throw mariadb_error("Out of memory", CR_OUT_OF_MEMORY, "HY000");
        _size = len;
        return _data;
    }

    void Bind::Buffer::free(bool& heap) {
        if (heap) std::free(_data);
        heap = false;
    }

    Bind::Bind() : _buffer(), _length(), _type(MYSQL_TYPE_NULL), _null(true), _unsigned(false), _error(false), _heap(false) {
    }
    ...
    void Bind::throw_unsupported_type() {
        throw mariadb_error("Using unsupported bind type", CR_UNSUPPORTED_PARAM_TYPE, "HY000");
    }
    ...
    Bind::operator MYSQL_BIND() {
        ...
        res.buffer_length = static_cast<unsigned long>(_buffer.size(_heap));
...
// Should be called only on error, if buffer was too small
    void Bind::realloc(unsigned long length) {
        _buffer.alloc(_heap, length);
    }

    Bind& Bind::init(MYSQL_FIELD* field) {
        ...
        switch (field->type) {
            ...
            case MYSQL_TYPE_GEOMETRY:
                _buffer.alloc(_heap, field->max_length);
                break;
                // Numeric types
            case MYSQL_TYPE_YEAR:
            case MYSQL_TYPE_TINY:
                _buffer.alloc(_heap, 1);
                break;
            case MYSQL_TYPE_SHORT:
                _buffer.alloc(_heap, 2);
                break;
            case MYSQL_TYPE_INT24:
            case MYSQL_TYPE_FLOAT:
            case MYSQL_TYPE_LONG:
                _buffer.alloc(_heap, 4);
                break;
            case MYSQL_TYPE_LONGLONG:
            case MYSQL_TYPE_DOUBLE:
                _buffer.alloc(_heap, 8);
                break;
                // Time types
            ...
            case MYSQL_TYPE_DATETIME:
                _buffer.alloc(_heap, sizeof(MYSQL_TIME));
                break;
            default:
    ...
    template<typename T>
    bool Bind::setNumeric(T value, const enum_field_types type, const bool is_uns) {
        void* const old = _buffer.data(_heap);
        void* data = _buffer.alloc(_heap, sizeof(T));
        *reinterpret_cast<T*>(data) = value;
        _null = false;
        if (type == _type && is_uns == static_cast<const bool>(_unsigned) && old == data) return false;
        _type = type;
        _unsigned = is_uns;
        return true;
    }

    bool Bind::setBuffer(const char* str, size_t len, enum_field_types type) {
        _null = !str;
        void* const old = _buffer.data(_heap);
    ...
    bool Bind::setTime(const Time& time, enum_field_types type) {
        const MYSQL_TIME& tm = time;
        return setBuffer(reinterpret_cast<const char*>(&tm), sizeof(MYSQL_TIME), type);
    }
    ...
    bool Bind::setDouble(double value) {
        return setNumeric(value, MYSQL_TYPE_DOUBLE, false);
    }

    bool Bind::setFloat(float value) {
        return setNumeric(value, MYSQL_TYPE_FLOAT, false);
    }

    bool Bind::setCString(const char* str) {
    ...
    bool Bind::setBlob(const std::string& str) {
        return setBuffer(str.data(), str.size(), MYSQL_TYPE_BLOB);
    }

    bool Bind::setDateTime(const Time& time) {
        switch (time.time_type) {
            case MYSQL_TIMESTAMP_DATE:
                return setTime(time, MYSQL_TYPE_DATE);
            case MYSQL_TIMESTAMP_TIME:
                return setTime(time, MYSQL_TYPE_TIME);
            case MYSQL_TIMESTAMP_DATETIME:
                return setTime(time, MYSQL_TYPE_DATETIME);
            default:
                break;
        }
    ...
    std::string Bind::getString() const {
        ...
        switch (_type) {
            ...
            case MYSQL_TYPE_FLOAT:
                ...
                break;
            case MYSQL_TYPE_DOUBLE:
                os << std::scientific << std::setprecision(17) << *reinterpret_cast<const double*>(data);
                break;
            case MYSQL_TYPE_TIMESTAMP:
            case MYSQL_TYPE_DATE:
            case MYSQL_TYPE_TIME:
    ...
    int64_t Bind::getBigInt() const {
        ...
        switch (_type) {
            ...
            case MYSQL_TYPE_NEWDATE:
            case MYSQL_TYPE_GEOMETRY:
                return from_chars<int64_t>(data, data + data_length());
            case MYSQL_TYPE_TINY:
            ...
            case MYSQL_TYPE_DOUBLE:
                return static_cast<int64_t>(*reinterpret_cast<const double*>(data));
            case MYSQL_TYPE_TIMESTAMP:
            case MYSQL_TYPE_DATE:
            case MYSQL_TYPE_TIME:
            case MYSQL_TYPE_DATETIME:
    ...
    uint64_t Bind::getUBigInt() const {
        ...
        switch (_type) {
            ...
            case MYSQL_TYPE_NEWDATE:
            case MYSQL_TYPE_GEOMETRY:
                return from_chars<uint64_t>(data, data + data_length());
            case MYSQL_TYPE_TINY:
            ...
            case MYSQL_TYPE_FLOAT:
                return static_cast<uint64_t>(*reinterpret_cast<const float*>(data));
            case MYSQL_TYPE_DOUBLE:
                return static_cast<uint64_t>(*reinterpret_cast<const double*>(data));
            case MYSQL_TYPE_TIMESTAMP:
            case MYSQL_TYPE_DATE:
            case MYSQL_TYPE_TIME:
            case MYSQL_TYPE_DATETIME:
    ...
    float Bind::getFloat() const {
        ...
        switch (_type) {
            ...
            case MYSQL_TYPE_NEWDATE:
            case MYSQL_TYPE_GEOMETRY:
                return from_chars<float>(data, data + data_length());
            case MYSQL_TYPE_TINY:
    ...
    double Bind::getDouble() const {
        ...
        switch (_type) {
            ...
            case MYSQL_TYPE_NEWDATE:
            case MYSQL_TYPE_GEOMETRY:
                return from_chars<double>(data, data + data_length());
            case MYSQL_TYPE_TINY:

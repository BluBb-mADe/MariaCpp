...
int poll(struct pollfd* pfds, unsigned long int nfds, int timeout) {
...
#   endif
    ...
    void Connection::connect(const Uri& uri, const char* usr, const char* passwd, unsigned long clientflag) {
        ...
        return connect(uri.host(), usr, passwd, uri.schema(), uri.port(), uri.socket(), clientflag);
    }
    ...
    void Connection::connect(const char* host, const char* user, const char* passwd, const char* db, unsigned int port, const char* unix_socket,
                             unsigned long clientflag) {
        if (!mysql_real_connect(&mysql, host, user, passwd, db, port, unix_socket, clientflag))
            throw_exception();
    }

    std::string Connection::escape_string(const std::string& s) {
        if (s.empty()) return s;
        std::string buffer(s.size() * 2 + 1, '\0');
        unsigned long len = escape_string(&buffer[0], s.c_str(), static_cast<unsigned long>(s.size()));
        buffer.resize(len);
        return buffer;
    }

    ResultSet* Connection::list_dbs(const char* wild) {
        MYSQL_RES* res = mysql_list_dbs(&mysql, wild);
        if (!res && errorno()) throw_exception();
        return res ? new ResultSet(*this, res) : nullptr;
    }

    ResultSet* Connection::list_fields(const char* table, const char* wild) {
        MYSQL_RES* res = mysql_list_fields(&mysql, table, wild);
        if (!res && errorno()) throw_exception();
        return res ? new ResultSet(*this, res) : 0;
    }

    ResultSet* Connection::list_processes() {
        MYSQL_RES* res = mysql_list_processes(&mysql);
        if (!res && errorno()) throw_exception();
        return res ? new ResultSet(*this, res) : 0;
    }

    ResultSet* Connection::list_tables(const char* wild) {
        MYSQL_RES* res = mysql_list_tables(&mysql, wild);
        if (!res && errorno()) throw_exception();
        return res ? new ResultSet(*this, res) : 0;
    }

    bool Connection::next_result() {
        CC();
    ...
    void Connection::options(enum mysql_option option, const void* arg) {
    ...
    void Connection::options4(enum mysql_option option, const void* arg1, const void* arg2) {
    ...
    std::string Connection::session_track_get_first(enum enum_session_state_type type) {
        CC();
        const char* data;
        size_t len;
    ...
    bool Connection::session_track_get_next(enum enum_session_state_type type, std::string& str) {
        CC();
        const char* data;
        size_t len;
    ...
    void
    Connection::set_local_infile_handler(int (* local_infile_init)(void**, const char*, void*), int (* local_infile_read)(void*, char*, unsigned int),
                                         void (* local_infile_end)(void*), int (* local_infile_error)(void*, char*, unsigned int), void* userdata) {
...
//
    ...
    void Connection::autocommit_start(bool mode) {
        assert(!_async_status);
        my_bool ret;
    ...
    void Connection::autocommit_cont(int status) {
        assert(_async_status);
        my_bool ret;
    ...
    void Connection::commit_cont(int status) {
        assert(_async_status);
        my_bool ret;
    ...
    void Connection::dump_debug_info_start() {
        assert(!_async_status);
        int ret;
    ...
    void Connection::dump_debug_info_cont(int status) {
        assert(_async_status);
        int ret;
    ...
    void Connection::connect_start(const Uri& uri, const char* usr, const char* passwd, unsigned long clientflag) {
        return connect_start(uri.host(), usr, passwd, uri.schema(), uri.port(), uri.socket(), clientflag);
    }

    void Connection::connect_start(const char* host, const char* user, const char* passwd, const char* db, unsigned int port, const char* unix_socket,
                                   unsigned long clientflag) {
        assert(!_async_status);
        MYSQL* ret;
        _async_status = mysql_real_connect_start(&ret, &mysql, host, user, passwd, db, port, unix_socket, clientflag);
        if (!_async_status && ret == nullptr) throw_exception();
    }

    void Connection::connect_cont(int status) {
        assert(_async_status);
        MYSQL* ret;
    ...
    ResultSet* Connection::list_fields_start(const char* table, const char* wild) {
        assert(!_async_status);
        MYSQL_RES* res;
    ...
    ResultSet* Connection::list_fields_cont(int status) {
        assert(_async_status);
        MYSQL_RES* res;
    ...
    bool Connection::next_result_start() {
        assert(!_async_status);
        int ret = 0;
    ...
    bool Connection::next_result_cont(int status) {
        assert(_async_status);
        int ret = 0;
    ...
    void Connection::query_start(const char* sql) {
        return query_start(sql, static_cast<unsigned long>(strlen(sql)));
    }

    void Connection::query_start(const char* sql, unsigned long length) {
        assert(!_async_status);
        int ret;
    ...
    void Connection::query_cont(int status) {
        assert(_async_status);
        int ret;
    ...
    void Connection::rollback_start() {
        assert(!_async_status);
        my_bool ret;
    ...
    void Connection::rollback_cont(int status) {
        assert(_async_status);
        my_bool ret;
    ...
    void Connection::select_db_start(const char* db) {
        assert(!_async_status);
        int ret;
    ...
    void Connection::select_db_cont(int status) {
        assert(_async_status);
        int ret;
    ...
    ResultSet* Connection::store_result_start() {
        assert(!_async_status);
        MYSQL_RES* res;
    ...
    ResultSet* Connection::store_result_cont(int status) {
        assert(_async_status);
        MYSQL_RES* res;
    ...
    void Connection::set_character_set_start(const char* csname) {
        assert(!_async_status);
        int ret;
    ...
    void Connection::set_character_set_cont(int status) {
        assert(_async_status);
        int ret;
    ...
    void Connection::change_user_start(const char* usr, const char* pas, const char* db) {
        assert(!_async_status);
        my_bool ret;
    ...
    void Connection::change_user_cont(int status) {
        assert(_async_status);
        my_bool ret;
    ...
    void Connection::send_query_start(const char* sql, unsigned long length) {
        assert(!_async_status);
        int ret;
    ...
    void Connection::send_query_cont(int status) {
        assert(_async_status);
        int ret;
    ...
    void Connection::read_query_result_start() {
        assert(!_async_status);
        my_bool ret;
    ...
    void Connection::read_query_result_cont(int status) {
        assert(_async_status);
        my_bool ret;
    ...
    void Connection::shutdown_start(enum mysql_enum_shutdown_level shutdown_level) {
        assert(!_async_status);
        int ret;
    ...
    void Connection::shutdown_cont(int status) {
        assert(_async_status);
        int ret;
    ...
    void Connection::refresh_start(unsigned int options) {
        assert(!_async_status);
        int ret;
    ...
    void Connection::refresh_cont(int status) {
        assert(_async_status);
        int ret;
    ...
    void Connection::kill_start(unsigned long pid) {
        assert(!_async_status);
        int ret;
    ...
    void Connection::kill_cont(int status) {
        assert(_async_status);
        int ret;
    ...
    void Connection::set_server_option_start(enum enum_mysql_set_option option) {
        assert(!_async_status);
        int ret;
    ...
    void Connection::set_server_option_cont(int status) {
        assert(_async_status);
        int ret;
    ...
    void Connection::ping_cont(int status) {
        assert(_async_status);
        int ret;
    ...
    const char* Connection::stat_start() {
        assert(!_async_status);
        const char* ret;
    ...
    const char* Connection::stat_cont(int status) {
        assert(_async_status);
        const char* ret;
    ...
    void Connection::reset_connection_start() {
        assert(!_async_status);
        int ret;
    ...
    void Connection::reset_connection_cont(int status) {
        assert(_async_status);
        int ret;

...
namespace MariaCpp {
    ...
    Uri::Uri(const std::string& str) : _protocol(PROTOCOL_DEFAULT), _port(DEFAULT_TCP_PORT), _host(), _schema() {
        parse(str);
    }

    const char* Uri::host() const {
        return _host.empty() || PROTOCOL_TCP != _protocol ? NULL : _host == "localhost" ? "127.0.0.1" : _host.c_str();
    }

    const char* Uri::socket() const {
        return _host.empty() || (PROTOCOL_PIPE != _protocol && PROTOCOL_SOCKET != _protocol) ? NULL : _host.c_str();
    }
    ...
    void Uri::set_socket(const std::string& name) {
        _protocol = PROTOCOL_SOCKET;
        _host = name;
    }

    void Uri::set_pipe(const std::string& name) {

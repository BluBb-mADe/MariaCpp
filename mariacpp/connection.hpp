...
namespace MariaCpp {
    ...
    public:
        ...
        unsigned long long affected_rows() { return mysql_affected_rows(&mysql); }

        void autocommit(bool mode) {
            CC();
            if (mysql_autocommit(&mysql, mode)) throw_exception();
        }

        void change_user(const char* usr, const char* pas, const char* db) {
            CC();
            if (mysql_change_user(&mysql, usr, pas, db)) throw_exception();
        }

        const char* character_set_name() { return mysql_character_set_name(&mysql); }

        void close();

        void commit() {
            CC();
            if (mysql_commit(&mysql)) throw_exception();
        }

        // mysql_connect() is deprecated.
        // In MariaCpp: connect() is an alias for real_connect().
        void connect(const Uri& uri, const char* usr, const char* passwd, unsigned long clientflag = 0);

        void connect(const char* host, const char* user, const char* passwd, const char* db, unsigned int port, const char* unix_socket,
                     unsigned long clientflag);

        void dump_debug_info() {
            CC();
            if (mysql_dump_debug_info(&mysql)) throw_exception();
        }

        unsigned int errorno() { return mysql_errno(&mysql); }

        const char* error_str() { return mysql_error(&mysql); }

        // mysql_escape_string() is forbidden.
        // This method is using mysql_real_escape_string()
        unsigned long escape_string(char* to, const char* from, unsigned long length) { return mysql_real_escape_string(&mysql, to, from, length); }

        std::string escape_string(const std::string& str);

        unsigned int field_count() { return mysql_field_count(&mysql); }

        void get_character_set_info(MY_CHARSET_INFO* cs) { return mysql_get_character_set_info(&mysql, cs); }

        const char* get_host_info() { return mysql_get_host_info(&mysql); }

#   if 50700 <= MYSQL_VERSION_ID

        void get_option(enum mysql_option option, void* arg) { if (mysql_get_option(&mysql, option, arg)) throw_exception(); }

#   endif

        unsigned int get_proto_info() { return mysql_get_proto_info(&mysql); }

        const char* get_server_info() { return mysql_get_server_info(&mysql); }

        unsigned long get_server_version() { return mysql_get_server_version(&mysql); }

        const char* get_ssl_cipher() { return mysql_get_ssl_cipher(&mysql); }

        const char* info() { return mysql_info(&mysql); }

        my_ulonglong insert_id() { return mysql_insert_id(&mysql); }

        void kill(unsigned long pid) {
            CC();
            if (mysql_kill(&mysql, pid)) throw_exception();
        }
        ...
        bool more_results() { return mysql_more_results(&mysql); }
...
#   endif
        ...
        void ping() {
            CC();
            if (mysql_ping(&mysql)) throw_exception();
        }
        ...
        std::string real_escape_string(const std::string& str) { return escape_string(str); }

        // In MariaCpp: real_connect() == connect()
        void real_connect(const Uri& uri, const char* usr, const char* passwd, unsigned long clientflag = 0) {
            return connect(uri, usr, passwd, clientflag);
        }

        void real_connect(const char* host, const char* user, const char* passwd, const char* db, unsigned int port, const char* unix_socket,
                          unsigned long cliflag) { return connect(host, user, passwd, db, port, unix_socket, cliflag); }

        // In MariaCpp: real_query(sql, length) == query(sql, length)
        void real_query(const char* sql, unsigned long length) { return query(sql, length); }

        void refresh(unsigned int options) {
            CC();
            if (mysql_refresh(&mysql, options)) throw_exception();
        }

        void reload() {
            CC();
            if (mysql_reload(&mysql)) throw_exception();
        }

#   if 50700 <= MYSQL_VERSION_ID

        void reset_connection() { if (mysql_reset_connection(&mysql)) throw_exception(); }

#   endif

        void rollback() {
            CC();
            if (mysql_rollback(&mysql)) throw_exception();
        }

        void select_db(const char* db) {
            CC();
            if (mysql_select_db(&mysql, db)) throw_exception();
        }
...
#   if 50700 <= MYSQL_VERSION_ID
        ...
        bool session_track_get_next(enum enum_session_state_type type, std::string& res);

#   endif

        void set_character_set(const char* csname) {
            CC();
            if (mysql_set_character_set(&mysql, csname)) throw_exception();
        }

        void set_local_infile_default() { return mysql_set_local_infile_default(&mysql); }

        void set_local_infile_handler(int (* local_infile_init)(void**, const char*, void*), int (* local_infile_read)(void*, char*, unsigned int),
        ...
        void set_server_option(enum enum_mysql_set_option option) {
            CC();
            if (mysql_set_server_option(&mysql, option)) throw_exception();
        }

        void shutdown(enum mysql_enum_shutdown_level shutdown_level) {
            CC();
            if (mysql_shutdown(&mysql, shutdown_level)) throw_exception();
        }

        const char* sqlstate() { return mysql_sqlstate(&mysql); }

        bool ssl_set(const char* key, const char* cert, const char* ca, const char* capath, const char* cipher) {
...
#   ifdef MARIADB_VERSION_ID
        ...
        void close_start() { _async_status = mysql_close_start(&mysql); }

        void close_cont(int status) { _async_status = mysql_close_cont(&mysql, status); }
        ...
        void connect_start(const Uri& uri, const char* usr, const char* passwd, unsigned long clientflag = 0);

        void connect_start(const char* host, const char* user, const char* passwd, const char* db, unsigned int port, const char* unix_socket,
                           unsigned long clientflag);

        void connect_cont(int status);

        my_socket get_socket() { return mysql_get_socket(&mysql); }

        unsigned int get_timeout_value() const { return mysql_get_timeout_value(&mysql); }

        unsigned int get_timeout_value_ms() const { return mysql_get_timeout_value_ms(&mysql); }
        ...
        void query_start(const std::string& sql) { return query_start(sql.data(), static_cast<unsigned long>(sql.size())); }
